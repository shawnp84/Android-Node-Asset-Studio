...
loadImageResources = function(images, document, callback) {
  var imageResources = {};
...
exports.loadImageResources = loadImageResources;

loadFromUri = function(uri, callback) {
...
loadXmlTemplates = function(files, callback) {
	var fileResources = {};
...
exports.loadXmlTemplates = loadXmlTemplates;
...
drawing.getTrimRect = function(ctx, size, minAlpha, callback) {
  callback = callback || function(){};
...
drawing.getCenterOfMass = function(ctx, size, minAlpha, callback) {
  callback = callback || function(){};
...
drawing.copyAsAlpha = function(dstCtx, src, size, onColor, offColor) {
  onColor = onColor || '#fff';
...
drawing.makeAlphaMask = function(ctx, size, fillColor) {
...
drawing.applyFilter = function(filter, ctx, size) {
  var src = ctx.getImageData(0, 0, size.w, size.h);
  var dst = ctx.createImageData(size.w, size.h);
...
(function() {
  ...
  function glfxblur_(radius, ctx, size) {
    ...
    drawing.blur = function(radius, ctx, size) {
    try {
...
drawing.fx = function(effects, dstCtx, src, size) {
  effects = effects || [];
  ...
  for (var i = 0; i < outerEffects.length; i++) {
    ...
    switch (effect.effect) {
      case 'outer-shadow':
        ...
        drawing.clear(tmpCtx, paddedSize);

        tmpCtx.save(); // T2
        tmpCtx.translate(padLeft, padTop);
        drawing.copyAsAlpha(tmpCtx, src.canvas || src, size);
        tmpCtx.restore(); // T2

        if (effect.blur)
          drawing.blur(effect.blur, tmpCtx, paddedSize);

        drawing.makeAlphaMask(tmpCtx, paddedSize, effect.color || '#000');
        if (effect.translate)
          dstCtx.translate(effect.translate.x || 0, effect.translate.y || 0);

        dstCtx.globalAlpha = Math.max(0, Math.min(1, effect.opacity || 1));
        dstCtx.translate(-padLeft, -padTop);
        drawing.copy(dstCtx, tmpCtx, paddedSize);
        break;
    }
  ...
  drawing.clear(tmpCtx, size);
  drawing.copy(tmpCtx, src.canvas || src, size);
  var fillOpacity = 1.0;
  ...
  drawing.copy(dstCtx, tmpCtx, size);
  ...
  tmpCtx = drawing.context(paddedSize);
  tmpCtx2 = drawing.context(paddedSize);
  ...
  for (var i = 0; i < innerEffects.length; i++) {
    ...
    switch (effect.effect) {
      case 'inner-shadow':
        ...
        tmpCtx.save(); // T2
        tmpCtx.translate(padLeft, padTop);
        drawing.copyAsAlpha(tmpCtx, src.canvas || src, size, '#fff', '#000');
        tmpCtx.restore(); // T2

        tmpCtx2.save(); // T2
        tmpCtx2.translate(padLeft, padTop);
        drawing.copyAsAlpha(tmpCtx2, src.canvas || src, size);
        tmpCtx2.restore(); // T2

        if (effect.blur)
          drawing.blur(effect.blur, tmpCtx2, paddedSize);
        drawing.makeAlphaMask(tmpCtx2, paddedSize, '#000');
        if (effect.translate)
          tmpCtx.translate(effect.translate.x || 0, effect.translate.y || 0);

        tmpCtx.globalCompositeOperation = 'source-over';
        drawing.copy(tmpCtx, tmpCtx2, paddedSize);

        drawing.makeAlphaMask(tmpCtx, paddedSize, effect.color);
        dstCtx.globalAlpha = Math.max(0, Math.min(1, effect.opacity || 1));
        dstCtx.translate(-padLeft, -padTop);
        drawing.copy(dstCtx, tmpCtx, paddedSize);
        break;
